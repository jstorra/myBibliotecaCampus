...
export const getAll = async ({uri, endpoint} = {}) => {
    if (!uri) return { status: 400, message: "Envia la URI por favor."}
    if (!endpoint) return { status: 400, message: "Envia el Endpoint por favor."}
    return await (await fetch(`${uri}${endpoint}`)).json();
} 
export const getOne = async ({id, uri, endpoint} = {}) => {
    if (!id) return { status: 400, message: "Envia el Id por favor."}
    if (typeof id !== 'number') return { status: 400, message: "El Id tiene un formato invalido."}
    if (!uri) return { status: 400, message: "Envia la URI por favor."}
    if (!endpoint) return { status: 400, message: "Envia el Endpoint por favor."}
